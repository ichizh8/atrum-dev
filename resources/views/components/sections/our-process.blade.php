    ...
    <x-container>
        <div class="grid grid-cols-9 gap-4 mb-10">
            <div class="col-span-9 relative">
                ...
                <h2 class="relative an_text-appear">Process — <span class="gradient-text">How We <br/>Keep You Out of Trouble</span></h2>
            </div>
        </div>
        <div class="grid grid-cols-9 gap-4 mb-8">
            <div class="col-span-3 max-smd:col-span-9 an_text-split">
                <h4 class="max-sm:mb-10 split" data-delay="0.3">Planned.<br />Modeled.<br/>Tested.<br/>Then Built.</h4>
            </div>
            <div class="col-span-1 col-start-5 flex items-end max-smd:col-span-9 max-smd:col-start-1 an_text-split">
                <h6 class="text-black/50 uppercase split" data-delay="0.5">Step-by-Step Process</h6>
            </div>
        </div>
                    ...
                    @foreach ($steps as $index => $step)
                        <li class="flex items-start our-process-item relative z-{{ count($steps) - $index  }}" >
                            ...
                            <div class="our-process-content">
                                <h5 class="mb-4 max-sm:mb-2">{{ $step['title'] }}</h5>
                                <ul class="snowflake-list">
                                    @foreach ($step['list'] as $item)
                                        <li class="text-black/60">{{ $item }}</li>
                                    @endforeach
                                </ul>
                            </div>
                        </li>
